...
const LOW_INTENT_RE =
  /\b(just looking|curious|maybe|thinking about|question|wondering|not sure|later|sometime|browsing)\b/i;
...
function detectIntentTag(content: string): ConversationIntelligenceTag {
  ...
  if (hasHighIntent || (hasUrgency && hasActionableTopic)) {
    return { kind: "high_intent", label: "High intent" };
  }

  if (hasLowIntent || hasResearchIntent) {
    return { kind: "low_intent", label: "Low intent" };
  }

  // Fallback: actionable asks with clear next-step language are high intent.
  const actionableNextStep = /\b(price|payment|quote|availability|appointment|book|schedule|test drive|service)\b/i;
